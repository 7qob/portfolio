...
`impressum.html` and the project pages open
...
Only the home page's vault panel, `login.html` and `admin.html` need the API, and each degrades
...
```
index.html          the site, one scrolling page, vault included
project-*.html      one page per project, generated by the panel
impressum.html      legal + Datenschutz
login.html          sign-in for the admin panel
