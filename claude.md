...
project-<slug>.html generated by the panel, linked from the home page's cards
404.html            not found
impressum.html      legal + Datenschutz, in the home page's own design:
                    same topbar, same rail, same numbered sections, same footer
login.html          sign-in, where admin.html sends a signed-out visitor
...
The two halves are deliberately separable. `index.html`, `impressum.html` and
the project pages are pure static and work over `file://` or on GitHub Pages
with no backend at all. Only the home page's vault panel, the login and the
admin page need the API, and each degrades to a plain message
...
- **Strings that exist only in JS use `t(en, de)`** rather than a key. Text that
  ...
  the vault rows).
- **The button is in the topbar markup** of every page, and `render.ts` emits
  it too; `script.js` only binds it.
- **Product names are not translated.** React, Rust, KoboldCpp, Vault, GitHub,
  Impressum and the project names read the same in both languages.
...
- **Three tones, and body copy is not a heading.** `--fg-strong` is what you
  scan (headings, labels, a `<b>` in a row), `--fg` is what you read, and
  `--fg-mute` is what you check (values, meta, captions). Both tokens are
  theme-aware, so the single page points `--text` at `--fg` rather than at a
  literal. It used to raise `--fg` to `#efefef`, which is exactly
  `--fg-strong`: every rule that separated strong from normal then separated
  nothing, and the page read as one grey field. A hard colour in the
  `body.is-single` token block is the way that happens again.
- **Five type steps, and no two of them the same size.** On the single page:
  `--fs-name` (the hero, and the only thing that clamps, because it is the only
  type big enough for the viewport to have an opinion), `--fs-subtitle` (a card
  name), `--fs-lede` (the one sentence that opens a page, chosen by position
  through `.section--top .prose p` because the hero's words come out of the
  CMS), `--fs-body`, then `--fs-caption` and `--fs-meta` for the two label
...
- **Two line weights, and each says one thing.** `--line` is the boundary
  ...
  `.facts--seam li`, `.doclist .doc + .doc`. Drawn at one weight, a section
  header and a row divider look identical, and the vault section ended up with
  six lines that all meant something different. A new divider inside a section
  takes the soft token; if it wants the strong one, it is really a new section.
- **A section's air sits above it, never below.** `.section` is
  `padding: calc(var(--space) * 5) 0 0`, and its header sits 2.5 units above
  its own content: twice as far from the section before as from the words
  below, which is what the eye actually groups on. Symmetric padding makes a
  gap that belongs to neither side. The last section on the page buys its own
  bottom padding back, through `.col:last-child .section:last-child`.
- **One blue, and it is not a signal.** `--select` is the page's only other
  loud colour: the selection highlight and the contributions graph's field,
  declared once and referenced twice. The accent marks one eyebrow at a time,
  and a selection covers whole paragraphs, so red under a sentence reads as an
...
- **There is no projects index and no `/vault/` page.** Both were removed once
  the home page carried the project cards and the vault itself. Do not bring
  either back without a link to it from the home page.
...
**Publish** writes **two** things server-side (`server/src/projects/
render.ts`): `project-<slug>.html` per published project, and `index.html`,
the home page, whose `.card` rows `projectCard()` renders. Pagers are derived
from `sort_order`, so neighbouring pages re-render on every publish and the
chain never goes stale.
...
The home page's own writing — the hero and the about section — is
...
the same measure and in the same type as the words it replaced. The pen is the
last item of the section's own `.split` header row, not a box floating at the
section's top right: absolute positioning there put it on top of the rule that
runs to the right edge, and the hairline came through the glyph. A section with
no header row (the hero) keeps the corner, through `.ed-pen--float`.
...
- **`text`, `heading` and `media`, on `top` and `about`** — the `SECTION_KEYS`
  registry in `server/src/sections/blocks.ts` is the whole list, because every
  key must match a marker pair a human put in the page. `readme` was one of
  them until the section it named was deleted; a key with no markers publishes
  into nothing. `#contact`'s and `#stack`'s rows are still hand-written, and so is
  every project card on the page, which the panel writes instead.
- **The picture block is the one thing on the home page the renderer draws.**
  It emits `.rig--solo`, and that is now the only `.rig` left in `style.css`:
  the rest of that component went with the readme section it was built for.
...
- **The vault markup must not name its documents.** The list comes from the API
  after authentication. It was hardcoded once, which told anyone who viewed
  source what documents existed. Do not put it back.
- **Documents are rows, actions are a band.** In the home page's vault the
  list is only ever documents: soft dividers between rows, nothing above the
  first and nothing below the last, a reserved arrow slot so the row with no
  file behind it keeps the column. Download all and sign out sit under it in
  `.vault-actions`, as plain `.btn`s with no rule and no right-hand column.
  Drawn as one more row, the archive read as a fourth document.
- **The vault lives on the home page only.** `vaultDocs()` in `script.js`
  names the documents, says who is signed in and that every download is
  logged, and offers the ZIP of all of them through `GET /vault/archive`. It
  draws only after a session check.
...
- **A markup change in the renderer needs a Publish to take effect**, or
  ...
  same Publish is what drops the brand square from the generated pages — the static
