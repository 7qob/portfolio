...
- **The vault on the home page is the readable one, `/vault/` is the covered
  one.** `vaultDocs()` in `script.js` names the documents, says who is signed
  in and that every download is logged, and offers the ZIP of all of them
  through `GET /vault/archive`. The redaction bars below stay on `/vault/`,
  where a shared screen is the likelier risk; the home panel only ever draws
  after a session check, exactly as that page does.
